(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
        ? factory(
              ...
              require('backoff-rxjs'),
              require('rxjs-connector'),
    ...
    function (
                ...
                return rxjs
                    .zip(
                        rxjs.interval(1000).pipe(
                            operators.startWith(), // @todo It should be okay to send an empty message.
                    ...
                    .pipe(
                        ...
                        operators.map(function (values) {
                            return (
                                values.reduce(function (sum, currentValue) {
        ...
        var createTimingProviderConstructor = function createTimingProviderConstructor(
            estimateOffset,
                ...
                _createClass__default['default'](TimingProvider, [
                        ...
                        value: function update(newVector) {
                            ...
                            this._updateRequestsSubject.next(
                                _objectSpread(
                                    _objectSpread(
                                        {},
                                        timingObject.translateTimingStateVector(
                                            this._vector,
                                            performance.now() / 1000 - this._vector.timestamp
                                        )
                                    ),
                                    timingObject.filterTimingStateVectorUpdate(newVector)
                                )
                            );
                    ...
                    },
                    {
                        ...
                        value: function _createClient() {
                            ...
                            var url = PROVIDER_ID_REGEX.test(this._providerIdOrUrl)
                                ? ''.concat(SUENC_URL, '?providerId=').concat(this._providerIdOrUrl)
                            ...
                            var dataChannelSubjects = rxjs
                                .concat(
                                    rxjs
                                        .from(subscribableThings.online())
                                        .pipe(operators$1.equals(true), operators.first(), operators.ignoreElements()),
                                    rxjs.defer(function () {
                                        return rxjsConnector.accept(url, subjectConfig);
                                    })
                                )
                                .pipe(
                                    backoffRxjs.retryBackoff({
                            ...
                            var updateSubjects = dataChannelSubjects.pipe(
                                operators.map(function (dataChannelSubject) {
                            ...
                            var currentlyActiveUpdateSubjects = updateSubjects.pipe(
                                operators.expand(function (updateSubject) {
                                    return updateSubject.pipe(
                                        operators.catchError(function () {
                                            return rxjs.EMPTY;
                                        }),
                                        operators.ignoreElements(),
                                        operators.endWith(updateSubject)
                                    );
                                }),
                                operators.scan(function (activeUpdateSubjects, activeUpdateSubject) {
                                    ...
                                    if (index > -1) {
                                        return [].concat(
                                            _toConsumableArray__default['default'](activeUpdateSubjects.slice(0, index)),
                                            _toConsumableArray__default['default'](activeUpdateSubjects.slice(index + 1))
                                        );
                                    }
                            ...
                            this._updateRequestsSubject
                                .pipe(operators.withLatestFrom(currentlyActiveUpdateSubjects))
                                .subscribe(function (_ref) {
                                    ...
                                    activeUpdateSubjects.forEach(function (activeUpdateSubject) {
                                        try {
                                            activeUpdateSubject.send(
                                                _objectSpread(
                                                    _objectSpread({}, vector),
                                                    {},
                                                    {
                            ...
                            this._remoteRequestsSubscription = updateSubjects
                                .pipe(
                                    operators.withLatestFrom(dataChannelSubjects),
                                    operators.mergeMap(function (_ref3, index) {
                            ...
                            this._remoteUpdatesSubscription = updateSubjects
                                .pipe(
                                    operators.withLatestFrom(dataChannelSubjects),
                                    operators.mergeMap(function (_ref5) {
                                        var _ref6 = _slicedToArray__default['default'](_ref5, 2),
                                            updateSubject = _ref6[0],
                                            dataChannelSubject = _ref6[1];

                                        return rxjs.combineLatest([updateSubject, estimateOffset(dataChannelSubject)]).pipe(
                                            operators.catchError(function (err) {
                                                // tslint:disable-next-line:no-console
                                ...
                                .subscribe(function (_ref11) {
                                    var _ref12 = _slicedToArray__default['default'](_ref11, 2),
                                        ...
                                        timeOrigin = _ref12$.timeOrigin,
                                        remoteTimestamp = _ref12$.timestamp,
                                        velocity = _ref12$.velocity,
                                        offset = _ref12[1];

                                    var timestamp = remoteTimestamp - offset; // && this._timeOrigin < timeOrigin || (this._timeOrigin === timeOrigin && this._vector.timestamp > timestamp)

                                    if (_this3._isMain) {
                                        var vector = timingObject.translateTimingStateVector(
                                            _this3._vector,
                                            performance.now() / 1000 - _this3._vector.timestamp
                                        );
        ...
        var timingProviderConstructor = createTimingProviderConstructor(
